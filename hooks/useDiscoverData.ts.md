...
- problem: Map needed the entire nearby dataset even when the card list paginates locally, but the hook only exposed `state.tokis`.
- solution: Added a second `mapEvents` state derived from `state.mapTokis` so map layers can always render all markers while the card list slices locally.
- Introduced `mapEvents` state + `previousMapEventIdsRef` that mirrors `state.mapTokis` via `transformTokiToEvent`, keeping a stable event list for the map even if the card list later slices the data.
