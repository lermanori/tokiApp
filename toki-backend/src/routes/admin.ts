...
const router = Router();
const BOOST_CODE_EXPIRY_HOURS = 72;
...
const requireBoostsEnabled = async (_req: Request, res: Response, next: NextFunction): Promise<void> => {
...
const generateBoostAuthorizationCode = () => {
  const token = crypto.randomBytes(4).toString('hex').toUpperCase();
  return `BOOST-${token}`;
};

const logBoostPurchaseRequestEvent = async (
