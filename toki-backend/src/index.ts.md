...
- problem: Root `/` endpoint missing, causing Railway healthcheck failures when checking root path.
- solution: Added root `/` endpoint that returns health check response, and updated all version references from '1.0.12' to '1.0.13'.
...
- Added root `/` GET endpoint before `/health` endpoint to handle Railway healthchecks that check the root path.
- Updated version strings in `/`, `/health`, and `/api` endpoints from '1.0.12' to '1.0.13' to match package.json version bump.
